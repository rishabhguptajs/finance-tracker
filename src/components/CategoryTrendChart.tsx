"use client";

import {
  Bar,
  BarChart,
  CartesianGrid,
  Legend,
  ResponsiveContainer,
  Tooltip,
  XAxis,
  YAxis,
} from "recharts";
import type { Category } from "@/lib/types";
import { CATEGORY_STYLES } from "@/lib/categories";
import { useChartTheme } from "@/lib/chart-theme";
import { formatINR } from "@/lib/format";

export interface CategoryTrendPoint {
  label: string;
  /** One key per category that has any spend in the window. */
  [category: string]: string | number;
}

function compactINR(value: number): string {
  if (Math.abs(value) >= 100000) return `₹${(value / 100000).toFixed(1)}L`;
  if (Math.abs(value) >= 1000) return `₹${Math.round(value / 1000)}k`;
  return `₹${value}`;
}

export default function CategoryTrendChart({
  data,
  categories,
}: {
  data: CategoryTrendPoint[];
  /** Already in the CVD-safe stacking order. */
  categories: Category[];
}) {
  const chart = useChartTheme();

  if (categories.length === 0) {
    return (
      <div className="flex h-64 items-center justify-center text-sm text-faint">
        Nothing logged in this range yet.
      </div>
    );
  }

  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={data} margin={{ top: 8, right: 4, left: -12, bottom: 0 }}>
        <CartesianGrid vertical={false} stroke={chart.grid} />
        <XAxis
          dataKey="label"
          tick={chart.axisTick}
          axisLine={false}
          tickLine={false}
        />
        <YAxis
          tick={chart.axisTick}
          axisLine={false}
          tickLine={false}
          tickFormatter={compactINR}
          width={56}
        />
        <Tooltip
          cursor={{ fill: chart.cursorFill }}
          formatter={(value, name) => [formatINR(Number(value)), String(name)]}
          contentStyle={chart.tooltip}
        />
        <Legend
          verticalAlign="top"
          align="left"
          height={32}
          iconType="circle"
          iconSize={8}
          formatter={(value) => (
            <span className="text-xs font-medium text-muted">{value}</span>
          )}
        />
        {categories.map((category, i) => (
          <Bar
            key={category}
            dataKey={category}
            stackId="spend"
            fill={CATEGORY_STYLES[category].hex}
            // A hairline of surface colour keeps touching segments legible.
            stroke={chart.surface}
            strokeWidth={2}
            maxBarSize={28}
            radius={i === categories.length - 1 ? [4, 4, 0, 0] : 0}
          />
        ))}
      </BarChart>
    </ResponsiveContainer>
  );
}
